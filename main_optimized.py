# GPU-Optimized Deepfake Detection - 100% RTX 4060 Utilization
# Optimized for maximum GPU usage with comprehensive metrics
...
def main():
    ...
    batch_size = 24  # Balanced for RTX 4060 8GB VRAM
    num_workers = min(6, mp.cpu_count())  # Optimal workers for stability
    ...
    optimizer = torch.optim.AdamW([
        {'params': backbone_params, 'lr': 1e-4, 'weight_decay': 1e-4},  # Lower LR for pretrained
        {'params': classifier_params, 'lr': 3e-4, 'weight_decay': 1e-4}  # Higher LR for new layers
    ], betas=(0.9, 0.999))

    # Learning rate scheduler - Fixed calculation
    ...
    print(f"Mixed Precision: Enabled, Warmup: {warmup_epochs} epochs")
    print(f"Model parameters: {sum(p.numel() for p in model.parameters()):,}")

    for epoch in range(num_epochs):
        start_time = time.time()
        
        # Warmup phase - reduce learning rate for first few epochs
        ...
        # Validation phase
        val_preds, val_labels, val_probs, val_loss = evaluate_model(model, val_loader, criterion, device, scaler)
    ...
    final_preds, final_labels, final_probs, final_loss = evaluate_model(model, val_loader, criterion, device, scaler)
